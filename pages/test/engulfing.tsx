...
import engulfing from '@/components/candlePattern/engulfing';
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
	...
	const kucoin = async () => {
		...
		for (let t=1; t<=60; t++) {
			...
			const fromDate = new Date(new Date().getTime() - t * 200 * interval * 60 * 1000).getTime();
			const toDate = new Date(fromDate + 200 * interval * 60 * 1000).getTime();
	...
	const conditionmet: series = []
	// data.map((ele, index, array) =>{
	for(let index = 0; index < data.length-1; index++){
        if (engulfing(data.slice(index, index+4))){
            conditionmet.push(data[index])
        }
    }
    return conditionmet;
        }
